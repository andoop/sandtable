...
## 背景 / 现状
- 插件结构：`skills/`（12 个 skill，含 autopilot 自动推进 skill）、`commands/` + `.cursor/commands/`（13 个 slash 命令，含 `/sandtable-autopilot`）、`templates/`（8 个模板）、`hooks/`（会话启动注入 `using-sandtable`）、`.claude-plugin/` + `.cursor-plugin/` + `.cursor/rules/`、`AGENTS.md`(=`CLAUDE.md`)。
