...
description: Use when all parallel implementation rehearsals returned DONE and you must objectively score them and pick the best one to integrate. Provides a scoring rubric and selection process, and routes any remaining anomaly back into the fix-and-re-rehearse loop.
---
...
<HARD-GATE>
仅当本轮所有实现预演子 agent 都返回 `DONE` 时才打分。任一 `ANOMALY_FOUND`/`BLOCKED` → 不打分，回到：主 agent 亲自核实 → 问开发者 → 修正 PRD/计划 → 重演。
</HARD-GATE>
