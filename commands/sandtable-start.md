---
description: 启动 Sandtable 沙盘推演驱动开发流程：从一句需求开始，澄清→PRD→计划→预演。
---
...
5. **PLAN**：加载 `writing-plan` 写 `plan.md`。〔= `/sandtable-plan`〕
6. 完成后提示我：可用 `/sandtable-refine` 反复完善，或用 `/sandtable-mental`→`/sandtable-redteam`→`/sandtable-live`→`/sandtable-debrief` 开始推演（`/sandtable-rehearse` 可一键串起）。
