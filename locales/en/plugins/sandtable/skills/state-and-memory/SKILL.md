...
1.5 **If `phase` ∈ {`DONE`, `FEEDBACK`} (post-landing)**: autopilot minimum coverage plus autonomous judgment does **not** apply; always **resume by `phase`**. `FEEDBACK` is human-in-the-loop, autopilot does not drive it, and it must **not** be mis-routed back to `EVALUATE` just because the three minimum coverage are met (advance manually via `/sandtable-bug`, `/sandtable-bugfix`).
